// MIT License
// https://github.com/smogon/pokemon-showdown-client/blob/master/play.pokemonshowdown.com/src/battle-log.ts

function usernameColor(name) {
    const hash = MD5(name + "");
    const H = parseInt(hash.substr(4, 4), 16) % 360; // 0..359
    const S = parseInt(hash.substr(0, 4), 16) % 50 + 40; // 40..89
    let L = Math.floor(parseInt(hash.substr(8, 4), 16) % 20 + 30); // 30..49
    ...
    const rgb2 = this.HSLToRGB(H, S, L);
    const r = rgb2.R, g = rgb2.G, b = rgb2.B;
...
function HSLToRGB(H, S, L) {
    ...
    const G = G1 + m;
    const B = B1 + m;
  return { R, G, B }; // values in 0..1
};

// MIT License
// https://github.com/smogon/pokemon-showdown-client/blob/master/play.pokemonshowdown.com/src/battle-log-misc.js
